...
import { OpenAI as PureOpenAI, params } from "./index"
...
describe("PureOpenAI", () => {
  let instance: InstanceType<typeof PureOpenAI>

  beforeEach(() => {
    instance = new PureOpenAI({ apiKey: "test" })

    // 使用 vi.spyOn 来模拟 chat.completions.create 方法
    vi.spyOn(instance["client"].chat.completions, "create").mockResolvedValue(new ReadableStream() as any)
  ...
  describe("responses.handlePayload", () => {
    ...
    it("should add search_context_size to web_search tool when OPENAI_SEARCH_CONTEXT_SIZE is set", async () => {
      ...
      expect(createCall.tools).toEqual(expect.arrayContaining([expect.objectContaining({ type: "web_search" })]))
    })
