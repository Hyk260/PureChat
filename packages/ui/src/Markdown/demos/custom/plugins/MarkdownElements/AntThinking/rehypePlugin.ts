...
const rehypePlugin = () => (tree: Node) => {
  visit(tree, "element", (node: any, index, parent) => {
    if (node.type === "element" && node.tagName === "p") {
      ...
      const openTagIndex = children.findIndex((child: any) => child.type === "raw" && child.value === "<antThinking>")
      const closeTagIndex = children.findIndex((child: any) => child.type === "raw" && child.value === "</antThinking>")
