  ...
  return (tree: any) => {
    visit(tree, "html", (node, index, parent) => {
      if (node.value === "<think>") {
        ...
        // 计算需要删除的节点范围
        const deleteCount = hasCloseTag ? endIndex - startIndex + 1 : parent.children.length - startIndex

        // 提取内容节点
        const contentNodes = parent.children.slice(startIndex + 1, hasCloseTag ? endIndex : undefined)
