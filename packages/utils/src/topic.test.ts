...
describe("groupTopicsByTime", () => {
  ...
  it("should sort groups in correct order", () => {
    ...
    expect(result.map((g) => g.id)).toEqual(["today", "yesterday", "week", dayjs(lastYear).year().toString()])
  })
  ...
  it("should sort topics within groups by createdAt in descending order", () => {
    ...
    const topics = [createTopic(today1, "Morning"), createTopic(today2, "Midday"), createTopic(today3, "Afternoon")]
