...
describe("base64 utilities", () => {
  describe("encodeToBase64", () => {
    it("should encode string to base64 in browser environment", () => {
      ...
      global.btoa = vi.fn().mockImplementation((input) => Buffer.from(input, "utf8").toString("base64"))
  ...
  describe("decodeFromBase64", () => {
    it("should decode base64 string in browser environment", () => {
      // Mock browser environment
      global.atob = vi.fn().mockImplementation((input) => Buffer.from(input, "base64").toString("utf8"))
  ...
  describe("round-trip encoding/decoding", () => {
    it("should preserve data through encode/decode cycle", () => {
      const testStrings = ["simple text", "test@123:password", "中文测试", "user:pass", "special!@#$%^&*()chars", ""]
