...
export async function loadThemeIfNeeded(highlighter: HighlighterGeneric<any, any>, theme: string): Promise<string> {
  ...
  if (!highlighter.getLoadedThemes().includes(theme)) {
    try {
      ...
      // 回退到 one-light
      console.warn(`Failed to load theme '${theme}', falling back to 'one-light':`, error as Error)
...
export async function getMarkdownIt(theme: string, markdown: string) {
  ...
  try {
    ...
    console.debug(`Failed to load theme '${theme}', using 'one-light' as fallback:`, error as Error)
