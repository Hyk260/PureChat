<template>
  <ElScrollbar>
    <ElCard class="style-header" shadow="never">
      ...
      <br />
      <TypewriterEffect
        :sentences="welcomeMessages"
        :typingSpeed="64"
        :pauseDuration="16_000"
        :loop="true"
        cursorStyle="underscore"
        :textColors="['#2dd4bf', '#f472b6', '#facc15']"
        cursorColor="#2dd4bf"
      />
    </ElCard>
    <!-- 项目信息 -->
    ...
    <ElCard class="style-card" shadow="hover">
      <template #header>
        <div>
          ...
          <ElButton class="ml-12" @click="startTyping"> 开始 </ElButton>
          <ElButton @click="restartTyping"> 重新开始 </ElButton>
          <ElButton @click="pauseTyping"> 暂停 </ElButton>
        </div>
      </template>
...
import { TypewriterEffect, Markdown } from "@pure/ui"
...
const markdownText = ref(fullMarkdownText.value)
...
const startTyping = () => {
  ...
  typingInterval.value = window.setInterval(() => {
    if (typingIndex.value < fullMarkdownText.value.length) {
      markdownText.value = fullMarkdownText.value.substring(0, typingIndex.value + 1)
      typingIndex.value++
    } else {
      clearInterval(typingInterval.value!)
      isTyping.value = false
    }
  }, 12) // 打字速度：50ms per char
}
...
const welcomeMessages = computed(() => {
  ...
  const welcomeData = currentLocale === "en" ? enWelcome : zhWelcome
  const messages = welcomeData?.welcome?.welcomeMessages || {}
  const messageValues = Object.values(messages)
  return shuffle(messageValues)
})
