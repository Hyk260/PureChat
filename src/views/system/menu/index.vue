    ...
    <el-container>
      ...
      <el-aside>
        <el-row class="mb-4">
          ...
          <el-button size="small" type="danger" @click="delMenuBtn" v-show="showDelBtn">删除</el-button>
        </el-row>
        <div class="input-style">
          <el-input class="me-input" placeholder="输入菜单名称搜索" v-model="filterText" clearable></el-input>
        </div>
              ...
              :filter-node-method="filterNode">
                <template #default="scope">
                  <div class="custom-node">
                    <FontIcon v-if="scope.data.meta.icon" :iconName="scope.data.meta.icon" />
      ...
      <el-main>
        ...
        <div class="Edit-menu">
          ...
          <!-- close-text="知道了" -->
          <el-row>
            <el-form ref="ruleLabelRef" :label-position="labelPosition" label-width="100px" :model="formLabelAlign">
              ...
              <el-form-item label="图标" prop="icon">
                <el-select v-model="formLabelAlign.icon" class="m-2 year" popper-class="style-select"
                  placeholder="请选择图标">
                  <el-option class="dropdown" v-for="item in ElIcons" :key="item.name" :label="item.name"
                    :value="item.name">
    ...
    <el-dialog v-model="dialogFormVisible" :title="'添加菜单'">
      ...
      <el-form ref="ruleFormRef" :model="ruleForm" status-icon :rules="rules" label-width="120px" class="demo-ruleForm">
        ...
        <el-form-item :label="ruleFormText.icon" prop="icon">
          <el-select v-model="ruleForm.icon" class="m-2 year" popper-class="style-select" placeholder="请选择图标">
            <el-option class="dropdown" v-for="item in ElIcons" :key="item.name" :label="item.name" :value="item.name">
              <FontIcon :iconName="item.name" />
            </el-option>
          </el-select>
        </el-form-item>
...
<script setup>
  ...
  const delMenuBtn = () => {
    const { meta:{ modify, title } } = state.treeData
